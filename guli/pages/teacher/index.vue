      ...
      <section class="c-sort-box unBr">
        ...
        <div>
          <div class="paging">
            ...
            <a v-for="page in data.pages" :key="page" :title="'第' + page + '页'" :class="{current: data.current === page, undisable: data.current === page}" href="#" @click.prevent="goPage(page)">{{ page }}</a>
...
export default {
  ...
  asyncData({ params, error }) {
    return teacher.pageListWeb(1, 8).then(response => {
      return { data: response.data.data }
    })
  },
  methods: {
    goPage(page) {
      teacher.pageListWeb(page, 8).then(response => {
